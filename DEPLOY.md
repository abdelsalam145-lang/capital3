...
export default function Dashboard() {
  ...
  // الربط بالـ API الحقيقي إن توفّر (يُضبط في متغير البيئة VITE_API_URL على Vercel).
  // مثال القيمة: https://your-laravel-api.com/api
  // ويجب توفّر رمز الدخول في VITE_API_TOKEN. بدونهما تُعرض البيانات النموذجية.
  useEffect(() => {
    const base = import.meta.env.VITE_API_URL;
    const token = import.meta.env.VITE_API_TOKEN;
    if (!base) return; // لا يوجد API مضبوط → اعرض النموذجية

    fetch(`${base}/dashboard`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
      .then((json) => { setData(json); setLive(true); })
      .catch(() => { /* فشل الاتصال → تبقى البيانات النموذجية */ });
  }, []);
    ...
    <div dir="rtl" style={{ background: `radial-gradient(1200px 500px at 80% -10%, ${C.bg2}, ${C.bg})`, color: C.white, fontFamily: "'IBM Plex Sans Arabic', sans-serif", minHeight: "100vh" }}>
      ...
      <main style={{ padding: 28, maxWidth: 1280, margin: "0 auto" }}>
        ...
        <footer style={{ textAlign: "center", color: C.silverDim, fontSize: 12.5, marginTop: 28, paddingTop: 16, borderTop: `1px solid ${C.cardLine}` }}>
          عاصمة الكون للمصاعد — {live
            ? "بيانات مباشرة من النظام"
            : <>بيانات نموذجية للعرض · تُربط بضبط <code style={{ background: C.card, padding: "1px 6px", borderRadius: 4, color: C.turq }}>VITE_API_URL</code></>}
        </footer>
      </main>
    </div>
  );
